#!/usr/bin/env bash
# ╔════════════════════════════════════════════════════════════════════╗
# ║  MediSign Psychology Adapter — RTX 4090 / Ada Lovelace Trainer    ║
# ║                                                                    ║
# ║  Cách dùng trên VM RTX 4090/A6000/3090 (Ada/Ampere):              ║
# ║                                                                    ║
# ║    export HF_TOKEN='hf_xxx'                                        ║
# ║    curl -sSL https://raw.githubusercontent.com/VNDT1625/\         ║
# ║      MediSign_AI/docs/fix-medgemma-model-name/scripts/cloud/\     ║
# ║      rtx4090_train_psychology.sh -o train.sh && bash train.sh      ║
# ║                                                                    ║
# ║  Optional env vars:                                                ║
# ║    MERGE_MEDICAL=1   → train on top of medical adapter            ║
# ║    NUM_EPOCHS=4      → number of training epochs                   ║
# ║                                                                    ║
# ║  ETA: ~1.5-2.5h trên RTX 4090 24GB (tùy flash-attn).              ║
# ║  Chi phí: ~$0.6-1.0 trên Vast.ai.                                  ║
# ╚════════════════════════════════════════════════════════════════════╝

set -euo pipefail

APP_DIR="${APP_DIR:-$HOME/medisign}"
PYTHON_SCRIPT_URL="${PYTHON_SCRIPT_URL:-https://raw.githubusercontent.com/VNDT1625/MediSign_AI/docs/fix-medgemma-model-name/scripts/cloud/rtx4090_train_psychology.py}"
LOG_FILE="$APP_DIR/training_psych.log"

GREEN='\033[0;32m'; BLUE='\033[0;34m'; RED='\033[0;31m'; YELLOW='\033[1;33m'; NC='\033[0m'
info()  { echo -e "${BLUE}[INFO]${NC} $*"; }
ok()    { echo -e "${GREEN}[ OK ]${NC} $*"; }
warn()  { echo -e "${YELLOW}[WARN]${NC} $*"; }
fail()  { echo -e "${RED}[FAIL]${NC} $*"; exit 1; }

banner() {
  echo ""
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║   $1"
  echo "╚════════════════════════════════════════════════════════════════╝"
}

# ─── Pre-flight checks ───────────────────────────────────────────────
banner "PRE-FLIGHT CHECKS"

[ -z "${HF_TOKEN:-}" ] && fail "HF_TOKEN chưa set. Chạy: export HF_TOKEN='hf_...'"
ok "HF_TOKEN: set (${#HF_TOKEN} chars)"

if ! command -v nvidia-smi &> /dev/null; then
  fail "nvidia-smi không có — không phải GPU instance?"
fi
GPU_NAME=$(nvidia-smi --query-gpu=name --format=csv,noheader | head -1)
GPU_VRAM=$(nvidia-smi --query-gpu=memory.total --format=csv,noheader,nounits | head -1)
ok "GPU: $GPU_NAME ($GPU_VRAM MB)"

# Detect GPU compute capability để biết flash-attn có khả thi không
GPU_CAP=$(nvidia-smi --query-gpu=compute_cap --format=csv,noheader | head -1)
ok "Compute capability: $GPU_CAP"

if ! command -v python3 &> /dev/null; then
  fail "python3 không có"
fi
PY_VER=$(python3 --version)
ok "$PY_VER"

mkdir -p "$APP_DIR"
cd "$APP_DIR"

# ─── Install deps ────────────────────────────────────────────────────
banner "STEP 1/4 — Install dependencies"

info "Upgrading pip..."
python3 -m pip install -q --upgrade pip --root-user-action=ignore 2>&1 | tail -2

info "Installing PyTorch + CUDA..."
python3 -m pip install -q --root-user-action=ignore \
  torch torchvision torchaudio \
  --index-url https://download.pytorch.org/whl/cu124 2>&1 | tail -3

info "Installing transformers + training stack..."
python3 -m pip install -q --root-user-action=ignore \
  "transformers>=4.50" "peft>=0.13" "bitsandbytes>=0.44" \
  "accelerate>=0.34" "trl>=0.12" "datasets>=3.0" \
  sentencepiece protobuf "huggingface_hub>=0.26" 2>&1 | tail -3

# ─── Flash Attention 2 (chỉ với Ada/Hopper, NOT Blackwell) ──────────
info "Installing Flash Attention 2..."

# Ada Lovelace = 8.9, Hopper = 9.0 → flash-attn pre-built có sẵn
# Blackwell = 12.0 → flash-attn KHÔNG có pre-built wheel
GPU_CAP_NUM=$(echo "$GPU_CAP" | sed 's/\.//')

if [ "$GPU_CAP_NUM" -ge 120 ]; then
  warn "Blackwell GPU detected (cap $GPU_CAP) → flash-attn không có pre-built wheel"
  warn "Sẽ dùng SDPA fallback (chậm hơn ~30%)"
  FLASH_OK=0
else
  PY_VER_SHORT=$(python3 -c "import sys; print(f'{sys.version_info.major}{sys.version_info.minor}')")
  TORCH_VER=$(python3 -c "import torch; v=torch.__version__.split('+')[0].split('.'); print(f'{v[0]}.{v[1]}')" 2>/dev/null || echo "")

  WHEEL_URL=""
  if [ "$PY_VER_SHORT" = "312" ] && [ "$TORCH_VER" = "2.4" ]; then
    WHEEL_URL="https://github.com/Dao-AILab/flash-attention/releases/download/v2.7.0.post2/flash_attn-2.7.0.post2+cu12torch2.4cxx11abiFALSE-cp312-cp312-linux_x86_64.whl"
  elif [ "$PY_VER_SHORT" = "312" ] && [ "$TORCH_VER" = "2.5" ]; then
    WHEEL_URL="https://github.com/Dao-AILab/flash-attention/releases/download/v2.7.0.post2/flash_attn-2.7.0.post2+cu12torch2.5cxx11abiFALSE-cp312-cp312-linux_x86_64.whl"
  elif [ "$PY_VER_SHORT" = "311" ] && [ "$TORCH_VER" = "2.4" ]; then
    WHEEL_URL="https://github.com/Dao-AILab/flash-attention/releases/download/v2.7.0.post2/flash_attn-2.7.0.post2+cu12torch2.4cxx11abiFALSE-cp311-cp311-linux_x86_64.whl"
  elif [ "$PY_VER_SHORT" = "311" ] && [ "$TORCH_VER" = "2.5" ]; then
    WHEEL_URL="https://github.com/Dao-AILab/flash-attention/releases/download/v2.7.0.post2/flash_attn-2.7.0.post2+cu12torch2.5cxx11abiFALSE-cp311-cp311-linux_x86_64.whl"
  fi

  FLASH_OK=0
  if [ -n "$WHEEL_URL" ]; then
    info "Trying pre-built wheel: $WHEEL_URL"
    if python3 -m pip install -q --root-user-action=ignore "$WHEEL_URL" 2>&1 | tail -3; then
      if python3 -c "import flash_attn; print(f'  Flash-Attn version: {flash_attn.__version__}')" 2>/dev/null; then
        ok "Flash Attention 2 installed (pre-built)"
        FLASH_OK=1
      fi
    fi
  fi

  if [ "$FLASH_OK" -eq 0 ]; then
    warn "Pre-built wheel không match (Python: $PY_VER_SHORT, PyTorch: $TORCH_VER)"
    warn "→ fallback SDPA"
  fi
fi

ok "Dependencies installed"

# ─── Download Python script ──────────────────────────────────────────
banner "STEP 2/4 — Download training script"

info "Pulling: $PYTHON_SCRIPT_URL"
curl -fsSL "$PYTHON_SCRIPT_URL" -o "$APP_DIR/rtx4090_train_psychology.py"
[ -s "$APP_DIR/rtx4090_train_psychology.py" ] || fail "Script download failed"
LINE_COUNT=$(wc -l < "$APP_DIR/rtx4090_train_psychology.py")
ok "Downloaded rtx4090_train_psychology.py ($LINE_COUNT lines)"

# ─── Run training ────────────────────────────────────────────────────
banner "STEP 3/4 — Training Psychology Adapter"

info "Logs at: $LOG_FILE"
info "ETA: ~1.5-2.5h tùy GPU + flash-attn"
info "Bắt đầu training..."
echo ""

cd "$APP_DIR"
HF_TOKEN="$HF_TOKEN" \
MERGE_MEDICAL="${MERGE_MEDICAL:-0}" \
NUM_EPOCHS="${NUM_EPOCHS:-4}" \
python3 rtx4090_train_psychology.py 2>&1 | tee "$LOG_FILE"
TRAIN_EXIT=${PIPESTATUS[0]}

if [ "$TRAIN_EXIT" -ne 0 ]; then
  fail "Training failed với exit code $TRAIN_EXIT. Xem log: $LOG_FILE"
fi

ok "Training completed!"

# ─── Done ────────────────────────────────────────────────────────────
banner "STEP 4/4 — Summary"

if [ -f "$APP_DIR/output_psych/adapter/adapter_config.json" ]; then
  ADAPTER_SIZE=$(du -sh "$APP_DIR/output_psych/adapter" | cut -f1)
  ok "Adapter saved: $APP_DIR/output_psych/adapter ($ADAPTER_SIZE)"
fi

if grep -q "HF push OK" "$LOG_FILE"; then
  ok "Pushed to: https://huggingface.co/thuaannn/medisign-medgemma4b-psychology"
fi

echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║                    🎉 PSYCHOLOGY DONE! 🎉                       ║"
echo "║  Adapter: https://huggingface.co/thuaannn/medisign-medgemma4b- ║"
echo "║           psychology                                            ║"
echo "║  → Destroy VM ngay để dừng tiền                                 ║"
echo "╚════════════════════════════════════════════════════════════════╝"
